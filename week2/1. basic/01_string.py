...
def is_palindrome(s):
    ...
    s_list = [ch for ch in s if ch.isdigit() or ch.isalpha()]
    final_s = "".join(s_list).lower()

    # TODO: 정제된 문자열이 회문인지 확인하세요
    # 방법1: 문자열을 뒤집어서 비교 ([::-1] 사용)
    # 방법2: 양 끝 인덱스를 이용한 투 포인터 방식
    
    #reversed_s = "".join(reversed(final_s))  #방법0
    #reversed_s = final_s[::-1]  #방법1
    '''
    if final_s == reversed_s:
        return True
    return False
    '''

    #방법2: 새 문자열을 생성하지 않아서 공간복잡도적으로 좋음 
    s_len = len(final_s)
    for i in range(int(s_len/2)):
        if final_s[i] != final_s[s_len-i-1]:
            return False
